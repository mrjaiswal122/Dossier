...
function UploadImage({ setUploadingImage }: props) {
  ...
  const handleUpload = async () => {
    if (image) {
      const uniqueKey =
        portfolio.routeName + "_" + (Date.now() + "_" + image.name);
      await dispatch(
        uploadImageAsync({
          routename: portfolio.routeName,
          image,
          key: uniqueKey,
          oldUrl: portfolio.personalInfo.profilePicture,
          type: Purpose.ProfileImage,
        })
      );
  ...
  const handleDelete = async () => {
    if (portfolio.personalInfo.profilePicture)
      await dispatch(
        deleteImageAsync({
          url: portfolio.personalInfo.profilePicture,
          deleteType: Purpose.ProfileImage,
          routeName: portfolio.routeName,
        })
      );
    await setUploadingImage(false);
  ...
  return (
    <section
      className="z-20 fixed w-full h-full top-0 left-0  dark:bg-black dark:bg-opacity-90 bg-white text-black bg-opacity-65"
      onClick={handleClick}
      id="uploadImage"
    >
      <section className="fixed top-[50%] left-[50%] w-[80vw] md:w-[60vw] lg: border  translate-x-[-50%] translate-y-[-50%] bg-white shadow-lg shadow-gray-500 border-gray-500 dark:bg-black rounded-lg flex flex-col justify-between ">
        ...
        <div className="mb-5 mx-3 text-sm">
          {/* delete button */}
          <button
            className="bg-reds flex justify-center items-center gap-2 p-2 rounded-lg float-left "
            disabled={!portfolio.personalInfo.profilePicture && !image}
            onClick={() => {
              if (image) {
                setImage(undefined);
                setImageUrl("");
              ...
              if (portfolio.personalInfo.profilePicture && !image) {
                setIsOpen(true);
                return;
              }
            }}
          ...
          <div className="float-right flex gap-3 items-center">
            ...
            {image ? (
              <button
                ...
                className="bg-greens flex justify-center items-center gap-2 p-2 rounded-lg  cursor-pointer"
              >
